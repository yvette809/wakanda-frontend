...
export const addExperience = (formData, history) => async (dispatch) => {
  try {
    ...
    const res = await axios.put("http://localhost:4000/profile/experience", formData, config);
    dispatch({
      type: UPDATE_PROFILE,
      payload: res.data,
    });
...
export const deleteExperience = (id) => async (dispatch) => {
  try {
    const res = await axios.delete(`http://localhost:4000/profile/experience${id}`);
    dispatch({
      type: UPDATE_PROFILE,
      payload: res.data,
    });
